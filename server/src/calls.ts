...
export async function computeCallees(tspClient: TspClient, args: lsp.TextDocumentPositionParams, documentProvider: DocumentProvider): Promise<lspcalls.CallsResult> {
    ...
    for (const reference of outgoingCallReferences) {
        ...
        const uri = pathToUri(definitionReference.file, undefined);
        const location = toLocation(reference, undefined);
        calls.push({
            location,
            symbol: { uri, ...definitionSymbol }
        });
    }
    return { calls, symbol: contextSymbol };
}

async function findOutgoingCalls(tspClient: TspClient, contextSymbol: lspcalls.DocumentSymbol, documentProvider: DocumentProvider): Promise<tsp.FileSpan[]> {
    ...
    const file = uriToPath(contextSymbol.uri!)!;
...
async function findEnclosingSymbol(tspClient: TspClient, args: tsp.FileSpan): Promise<lspcalls.DocumentSymbol | undefined> {
    ...
    return <lspcalls.DocumentSymbol>{ uri, ...symbol };
}
